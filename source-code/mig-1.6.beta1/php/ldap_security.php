...
function lookup_admin($conn_id, $user, $password)
{
	...
	$filter = "(&" . USERS_SEARCHFILTER . "(MIGCategory=MIGadmin)(|(uid=$user)(mail=$user)(mailAlternateAddress=$user)))";
...
{		
	$filter = "(&" . USERS_SEARCHFILTER . "(|(uid=$user)(mail=$user)(mailAlternateAddress=$user))(|(MigCategory=MIGuser)(MigCategory=MIGadmin)))";
	$sr = ldap_search ($conn_id, USERS_BASE_DIR, $filter, array("migcategory", "uid"));
	$result = ldap_get_entries ($conn_id, $sr);
