...
# LOWLEVEL
	...
	function read($from, $what, $result_array = "everything")
	{
		...
		else 
		{
			...
			else return $this->quit_on_error("More than one entry was return for a read operation : $from");
		}
	}
...
//    echo "Failed to set protocol version to 3";
	...
	function connect_to_datastore($uid = "auto", $passwd = "auto")
	{
		...
		else $this->conn_id = ldap_connect(HOSTNAME);
		...
		if ($uid == "auto")
		{
			if (defined("ADMIN") && (ADMIN == true))
			{
				if (defined("REMOTE_MANAGER_UID") && defined("REMOTE_MANAGER_PWD"))
				{
					if (!ldap_bind($this->conn_id, REMOTE_MANAGER_UID, REMOTE_MANAGER_PW)) return $this->quit_on_error("Unable to bind as a manager to datastore host : error " . ldap_errno($this->conn_id) ." : " . ldap_error($this->conn_id));
				}
				else if (!ldap_bind($this->conn_id, MANAGER_UID, MANAGER_PW)) return $this->quit_on_error("Unable to bind as a manager to datastore host : error " . ldap_errno($this->conn_id) ." : " . ldap_error($this->conn_id));
			}
			else
			{
