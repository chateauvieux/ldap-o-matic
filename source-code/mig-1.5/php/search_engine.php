...
//		echo "sort in " . ($end_time - $middle_time);
	...
	function display_results_pages($current_page = 1)
	{
		if ($this->count("result") > 0)
		{
			...
			if ($print_previous) echo "<a href=\"$SCRIPT_NAME?page=" . ($current_page - 1) . "&$postString\">Previous</a> ";
			...
			if ($print_next || print_previous)
			{
				for ($i = $start; (($i < (int) $this->count("result") / 30) && ($i < $current_page + 10 - 1)); $i++)
				{
					...
					if ($pagenum != $current_page)
						echo "<a href=\"$SCRIPT_NAME?page=$pagenum&$postString\">$pagenum</a> ";
					else echo $pagenum . " ";
				}  
			}		
			if ($print_next) echo "<a href=\"$SCRIPT_NAME?page=" . ($current_page + 1) . "&$postString\">Next</a> ";
	...
	function display_results($icon_path, $link_path, $pagenumber = 1)
	{
		...
		{ 
			...
			{			
				...
				{
					echo "<td rowspan=\"2\"><a href=\"$SCRIPT_NAME?sortattribute=" . $attr->migattributename . "&sortorder=asc&$postString\"><img border=\"0\" src=\"images/arrow_up.gif\"></a><br><a href=\"$SCRIPT_NAME?sortattribute=" . $attr->migattributename . "&sortorder=desc&$postString\"><img border=\"0\" src=\"images/arrow_down.gif\"></a></td>";
